...
ATT_RESOLVERS=(
    ...
    205.152.144.24    # BellSouth/ATT-SE — Miami, FL
    205.152.132.23    # BellSouth/ATT-SE — LA/MS/TN
)
# Control: a non-.co domain through the same resolvers.
CONTROL_DOMAIN=example.com

# Local resolvers: whatever the system is actually configured to use.
# Pulled from /etc/resolv.conf at startup so loops pick up the value once.
mapfile -t LOCAL_RESOLVERS < <(awk '/^nameserver/ {print $2}' /etc/resolv.conf 2>/dev/null)
TIMEOUT=3
TRIES=1
...
run_once() {
    ...
    log "-- Local resolvers (from /etc/resolv.conf) --"
    if [[ ${#LOCAL_RESOLVERS[@]} -eq 0 ]]; then
        log "  (none found in /etc/resolv.conf)"
    else
        for r in "${LOCAL_RESOLVERS[@]}"; do
            for dom in "${SAMPLE_DOMAINS[@]}"; do
                local result
                result=$(probe "$r" "$dom")
                log "  ${r} ${dom} ${result}"
